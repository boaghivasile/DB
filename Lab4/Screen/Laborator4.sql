...
SELECT distinct dis.Disciplina, (SELECT AVG(sr1.NOTA) from studenti_reusita AS sr1 WHERE sr.Id_Disciplina = sr1.Id_Disciplina) as Media
FROM studenti_reusita AS sr
JOIN discipline as dis
ON sr.Id_Disciplina = dis.Id_Disciplina
WHERE  (SELECT AVG(sr1.Nota) from studenti_reusita AS sr1 WHERE sr.Id_Disciplina = sr1.Id_Disciplina) > 7
