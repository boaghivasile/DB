/*Ex1.
Să se creeze proceduri stocate în baza exercițiilor (2 exerciții) din capitolul 4. 
Parametrii de intrare trebuie să corespundă criteriilor din clauzele WHERE ale exercițiilor respective.
*/
...
/*Ex.2 
Să se creeze o procedură stocată, care nu are niciun parametru de intrare si posedă un parametru de ieșire. 
Parametrul de ieșire trebuie să returneze numărul de studenți, 
care nu au susținut cel puțin o formă de evaluare (nota mai mică de 5 sau valoare NULL).
*/
...
/*Ex.3 
Să se creeze o procedură stocată, care ar însera în baza de date informații despre un student nou. 
În calitate de parametri de intrare să servească datele personale ale studentului nou și Cod_ Grupa. 
Să se genereze toate intrările-cheie necesare în tabelul studenti_reusita. Notele de evaluare să fie inserate ca NULL.
*/
...
SELECT * FROM grupe.grupe;
SELECT * FROM cadre_didactice.profesori;
SELECT * FROM plan_studii.discipline;
SELECT * FROM studenti.studenti;

/*Ex.4 
Fie că un profesor se eliberează din funcție la mijlocul semestrului. 
Să se creeze o procedură stocată care ar reatribui înregistrările din tabelul studenti_reusita unui alt profesor. 
Parametri de intrare: numele și prenumele profesorului vechi, numele și prenumele profesorului nou, disciplina. 
În cazul în care datele înserate sunt incorecte sau incomplete, să se afișeze un mesaj de avertizare.
*/
...
/*Ex.5
Să se creeze o procedură stocată care ar forma o listă cu primii 3 cei mai buni studenți la o disciplină, 
și acestor studenți să le fie marită nota la examenul final cu un punct (nota maximală posibilă este 10). 
În calitate de parametru de intrare, va servi denumirea disciplinei. 
Procedura să returneze următoarele câmpuri: Cod_ Grupa, Nume_Prenume_Student, Disciplina, Nota_Veche, Nota_Noua.
*/
...
/*Ex.6
Să se creeze funcții definite de utilizator în baza exercițiilor (2 exerciții) din capitolul 4. 
Parametrii de intrare trebuie să corespundă criteriilor din clauzele WHERE ale exercițiilor respective.
*/
...
/*Ex.7
Să se scrie funcția care ar calcula vârsta studentului. Să se definească următorul format al funcției: ().
*/
...
/*Ex.8
Să se creeze o funcție definită de utilizator, care ar returna datele referitoare la reușita unui student. 
Se definește următorul format al funcției: (<Nume_Prenume_Student>). 
...
/*Ex.9
Se cere realizarea unei funcții definite de utilizator, care ar găsi cel mai sârguincios sau cel mai slab student dintr-o grupă. 
Se definește următorul format al funcției: (<Cod_ Grupa>, <is_good>). 
Parametrul <is_good> poate accepta valorile "sarguincios" sau "slab", respectiv. 
Funcția să returneze un tabel cu următoarele câmpuri Grupa, Nume_Prenume_Student, Nota Medie , is_good. 
Nota Medie să fie cu precizie de 2 zecimale.
*/
...
SELECT * FROM BestOrLoser('INF171', 'Sarguincios')
